...
#include "fix_press_berendsen_active.h"
...
FixPressBerendsenActive::FixPressBerendsenActive(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  id_press(nullptr), pflag(0)
{
  if (narg < 5) error->all(FLERR,"Illegal fix press/berendsenactive command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"iso") == 0) {
      if (iarg+4 > narg)
        error->all(FLERR,"Illegal fix press/berendsenactive command");
      pcouple = XYZ;
      ...
      iarg += 4;
      if (dimension == 2)
        error->all(FLERR,"Invalid fix press/berendsenactive for a 2d simulation");
      ...
      else error->all(FLERR,"Illegal fix press/berendsenactive command");
      iarg += 2;
  ...
  if (pcouple == XYZ && (p_flag[0] == 0 || p_flag[1] == 0))
    error->all(FLERR,"Invalid fix press/berendsenactive pressure settings");
  ...
  if (p_flag[0] && domain->xperiodic == 0)
    error->all(FLERR,
               "Cannot use fix press/berendsenactive on a non-periodic dimension");
  ...
  if ((p_flag[0] && p_period[0] <= 0.0) ||
    ...
    error->all(FLERR,"Fix press/berendsenactive damping parameters must be > 0.0");
...
FixPressBerendsenActive::~FixPressBerendsenActive()
{
  delete [] rfix;
...
int FixPressBerendsenActive::setmask()
{
  int mask = 0;
  mask |= END_OF_STEP;
  return mask;
...
void FixPressBerendsenActive::init()
{
  if (domain->triclinic)
    error->all(FLERR,"Cannot use fix press/berendsenactive with triclinic box");
  ...
  for (int i = 0; i < modify->nfix; i++)
    if (strcmp(modify->fix[i]->style,"deform") == 0) {
      ...
      if ((p_flag[0] && dimflag[0]) || (p_flag[1] && dimflag[1]) ||
          (p_flag[2] && dimflag[2]))
        error->all(FLERR,"Cannot use fix press/berendsenactive and "
  ...
  if (icompute < 0)
    error->all(FLERR,"Pressure ID for fix press/berendsenactive does not exist");
...
/* ----------------------------------------------------------------------
   compute P before integrator starts
------------------------------------------------------------------------- */

void FixPressBerendsenActive::setup(int /*vflag*/)
{
...
void FixPressBerendsenActive::end_of_step()
{
  // compute new T,P
...
void FixPressBerendsenActive::couple()
{
...
void FixPressBerendsenActive::remap()
{
  int i;
...
int FixPressBerendsenActive::modify_param(int narg, char **arg)
{
